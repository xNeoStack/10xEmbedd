...
class InferenceApp:
    def __init__(self, root):
        ...
        self.log_trt_version()

        # Handle window closing
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def log_trt_version(self):
        """Log the installed TensorRT version."""
        try:
            version = trt.__version__
        except AttributeError:
            version = "Unknown"
        self.log_output(f"TensorRT Version: {version}", level='INFO')

    def log_output(self, message, level='INFO'):
        """Thread-safe method to log messages to the output_text widget and console."""
        self.gui_queue.put(message)
        # Optionally, integrate with Python's logging module
        # For simplicity, we're just printing to the console here
    ...
    def browse_model_file(self):
        ...
        if model_path:
            ...
            self.log_output(f"Selected model file: {model_path}", level='INFO')

    def browse_dataset_folder(self):
        dataset_folder = filedialog.askdirectory()
        if dataset_folder:
            self.dataset_path.set(dataset_folder)
            self.log_output(f"Selected dataset folder: {dataset_folder}", level='INFO')

    def get_num_bindings(self, engine):
        """Retrieve the number of bindings in a TensorRT engine, compatible with different TensorRT versions."""
        if hasattr(engine, 'num_bindings'):
            return engine.num_bindings  # TensorRT 8.x and above
        elif hasattr(engine, 'get_nb_bindings'):
            return engine.get_nb_bindings()  # TensorRT 7.x and below
        else:
            # Fallback: Attempt to count bindings by iterating
            try:
                count = 0
                while True:
                    engine.get_binding_name(count)
                    count += 1
            except trt.RuntimeError:
                self.log_output(f"Total bindings found via iteration: {count}", level='INFO')
                return count
            except Exception as e:
                self.log_output(f"Unexpected error while counting bindings: {e}", level='ERROR')
                raise AttributeError("ICudaEngine object has neither 'num_bindings' nor 'get_nb_bindings' attributes.")

    def get_num_bindings_safe(self, engine):
        """
        Safely retrieve the number of bindings.
        """
        try:
            return self.get_num_bindings(engine)
        except Exception as e:
            self.log_output(f"Error retrieving number of bindings: {e}", level='ERROR')
            # Optionally, list available attributes for debugging
            attributes = dir(engine)
            self.log_output(f"Available attributes/methods in engine: {attributes}", level='DEBUG')
            return 0
    ...
    def build_engine_from_onnx(self, onnx_file_path):
        ...
        self.log_output(f"Model bindings: {binding_names}", level='INFO')

        # Build the engine
        self.log_output("Building TensorRT engine from ONNX model...", level='INFO')
        engine = builder.build_engine(network, config)
        if engine is None:
            raise RuntimeError("Failed to build the TensorRT engine from ONNX model.")
        self.log_output("TensorRT engine successfully built.", level='INFO')

        # Log engine bindings
        num_bindings = self.get_num_bindings_safe(engine)
        if num_bindings > 0:
            binding_names = [engine.get_binding_name(i) for i in range(num_bindings)]
            self.log_output(f"Engine bindings after building: {binding_names}", level='INFO')
        else:
            self.log_output("No bindings found in the built engine.", level='WARNING')
        ...
        try:
            if model_path.endswith('.engine'):
                ...
                with open(model_path, "rb") as f, trt.Runtime(TRT_LOGGER) as runtime:
                    ...
                    self.log_output("TensorRT engine successfully loaded from .engine file.", level='INFO')

                    # Log all binding names for verification
                    num_bindings = self.get_num_bindings_safe(engine)
                    if num_bindings > 0:
                        binding_names = [engine.get_binding_name(i) for i in range(num_bindings)]
                        self.log_output(f"Engine bindings: {binding_names}", level='INFO')
                    else:
                        self.log_output("No bindings found in the loaded engine.", level='WARNING')
            ...
            self.log_output(f"Error loading engine: {str(e)}", level='ERROR')
            # Log available attributes/methods for debugging
    ...
    def load_images_from_folder(self, folder_path):
        ...
        for img_path in image_files:
            try:
                ...
                self.log_output(f"Error loading image {img_path}: {e}", level='ERROR')
        self.log_output(f"Loaded {len(images)} images from dataset.", level='INFO')
        return images
    ...
    def perform_inference(self):
        try:
            # Load images
            self.log_output("Loading images...", level='INFO')
            ...
            self.log_output(f"Total batches to process: {self.total_batches}", level='INFO')
            ...
            self.log_output(f"Error during inference setup: {e}", level='ERROR')
    ...
    def enqueue_batches(self):
        ...
        for batch_start in range(0, len(self.dataset_images), batch_size):
            ...
            self.log_output(f"Enqueued batch {batch_start // batch_size + 1}", level='INFO')
        self.log_output("All batches enqueued.", level='INFO')

    def monitor_gpu_usage(self):
        try:
            handle = pynvml.nvmlDeviceGetHandleByIndex(0)
        except pynvml.NVMLError as e:
            self.log_output(f"NVML Error: {e}", level='ERROR')
            return
        ...
        while self.inference_running:
            try:
                ...
                self.log_output(f"NVML Monitoring Error: {e}", level='ERROR')
                break
    ...
    def get_binding_index_safe(self, binding_name, default_idx):
        ...
        try:
            ...
            self.log_output(f"Binding '{binding_name}' not found. Using default binding index {default_idx}.", level='WARNING')
            return default_idx
    ...
    def inference_worker(self):
        ...
        try:
            ...
            self.log_output(f"{threading.current_thread().name}: CUDA context created.", level='INFO')

            # Create execution context
            execution_context = self.engine.create_execution_context()

            # Get the number of bindings
            num_bindings = self.get_num_bindings_safe(self.engine)
            if num_bindings == 0:
                self.log_output(f"{threading.current_thread().name}: No bindings available. Skipping inference.", level='ERROR')
                return
            ...
            while True:
                try:
                    ...
                    self.log_output(f"{threading.current_thread().name}: No more batches to process.", level='INFO')
                ...
                if -1 in input_shape:
                    try:
                        ...
                        self.log_output(f"{threading.current_thread().name}: Error setting binding shape: {e}", level='ERROR')
                        continue
                ...
                try:
                    ...
                    self.log_output(f"{threading.current_thread().name}: Inference execution failed: {e}", level='ERROR')
            ...
            self.log_output(f"{threading.current_thread().name}: Error during inference: {e}", level='ERROR')
            self.inference_running = False
            ...
            try:
                context.pop()
                del context
                self.log_output(f"{threading.current_thread().name}: CUDA context destroyed.", level='INFO')
            except Exception as e:
                self.log_output(f"{threading.current_thread().name}: Error destroying CUDA context: {e}", level='ERROR')
    ...
    def visualize_predictions(self, batch_images, predictions, batch_number, num=5):
        ...
        try:
            ...
            self.log_output(f"Error during visualization: {e}", level='ERROR')
    ...
    def finalize_inference(self):
        ...
        self.log_output(metrics, level='INFO')
        self.log_output("Inference completed for all batches.", level='INFO')
    ...
    def on_closing(self):
        if self.inference_running:
            if messagebox.askokcancel("Quit", "Inference is running. Do you want to quit?"):
                self.inference_running = False
                self.root.destroy()
        else:
            self.root.destroy()
